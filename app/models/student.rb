class Student < ApplicationRecord
  ...
  scope :ranking, -> { order('rating DESC') }
  ...
  def eligible(camp,student)
    camps_assigned = self.camps
    overlapping_camps = camps_assigned.where(start_date: camp.start_date, time_slot: camp.time_slot)
    if ((camp.curriculum.min_rating..camp.curriculum.max_rating).cover?(student.rating) and overlapping_camps.empty?)
      return true
    else 
      return false
    end
  ...
  def self.for_camp(camp)
    Registration.where(camp_id: camp.id).map{ |cs| cs.student }
  end
