class Ability
  ...
  def initialize(user)
    ...
    if user.role ==  "Parent"
      ...
      #see childrens transactions
      #see own and childrens accounts

      can :read, User do |u|
        my_kids = u.find_children
        found = false
        User.all.each do |kid|
          if(my_kids.include?(kid)) then found = true
        end
        found
      end

      can :read, Transaction do |t|
        my_transactions = user.get_transactions + user.get_children_transactions
        my_transactions.flatten!
        found = false
        Transaction.all.each do |trans|
          if(my_transactions.include?(trans)) then found = true
        end
        found
      end
      ...
      #can create own transfer
      #can see own accounts
      can :update, User do |u|  
        u.id == user.id
      end
      can :edit, User do |u|  
